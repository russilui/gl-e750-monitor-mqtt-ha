...
## Modified lines to add regular mcu monitoring over mqtt in alert script
# This probably needs to be a separate script to publish data regularly
# otherwise alert messages are published on the same data topics
publish_mcu_data() {
    ! is_connected && return 1
    $ALERT_SCRIPT $@
}

# print variables of interest (debug only)
echo {$TEMP, $BAT, $CHARGE}

# Send temp message (°C)
rm -f $TEMP_LOCK 
publish_mcu_data "$TEMP $BAT $CHARGE" && touch $TEMP_LOCK
